...
import {
  ...
  getEntriesForPeriod as mockGetEntriesForPeriod,
  getEntriesForAccount as mockGetEntriesForAccount,
  type MockAccount,
  type MockEntry,
} from "../services/mock-data.js"
import {
  getAllAccounts,
  getEntriesByPeriodWithFilters,
  getEntriesByPeriodRange,
  getEntriesByAccount,
  getReportLines,
  getCostCenters,
  getBudgetByPeriodRange,
  type DbAccount,
  type DbReportLine,
} from "@pala-dre/db"
...
function dbAccountToAccount(row: DbAccount): AccountLike {
  return {
    code: row.code,
    name: row.name,
...
function mockAccountToAccount(m: MockAccount): AccountLike {
  return {
    code: m.code,
    name: m.name,
...
function mockEntryToEntry(m: MockEntry): EntryLike {
  return {
    id: m.id,
...
function buildCategoryTree(accounts: AccountLike[]): DreCategory[] {
...
function sumDirectEntries(
  ...
  if (categories && categories.length > 0) {
    const visible = categories.some(
      (cat) => accountCode === cat || accountCode.startsWith(cat + ".")
    )
    if (!visible) return 0
  }
...
function computeAccountValue(
  ...
  if (valuesMap.has(code)) return valuesMap.get(code)!
  ...
  if (children.length === 0) {
    ...
    value = children.reduce(
      (acc, child) => acc + computeAccountValue(child.code, entries, accounts, valuesMap, categories),
      0
    )
  }
...
function computeAllValues(
  ...
  const accountValuesMap = new Map<string, number>()
...
function buildRowsFromLines(
  lines: DbReportLine[],
  valuesMap: Map<string, number>,
  comparativeValuesMap: Map<string, number> | null,
  budgetMap: Map<string, number> | null,
  receitaLiquidaValue: number,
  comparativeRL: number | null
): DreRow[] {
  const rows: DreRow[] = []
  ...
  for (const line of lines) {
    ...
    const avPercentComparative =
      compValue !== undefined && comparativeRL !== null && comparativeRL !== 0
        ? (compValue / comparativeRL) * 100
        : null
    const ahPercent =
      compValue !== undefined && compValue !== 0
    ...
    const budgetVariancePercent =
      budgetValue !== undefined && budgetValue !== 0
...
function buildBudgetMapFromLines(
  lines: DbReportLine[],
  budgetEntries: { account_code: string; amount: string }[],
  accounts: AccountLike[]
): Map<string, number> {
  ...
  const budgetEntryLikes: EntryLike[] = Array.from(rawMap.entries()).map(([code, amount], idx) => ({
    id: idx,
    accountCode: code,
    period: "",
    amount,
...
function generatePeriodList(periodFrom: string, periodTo: string): string[] {
  const periods: string[] = []
  const [fromYear, fromMonth] = periodFrom.split("-").map(Number) as [number, number]
  const [toYear, toMonth] = periodTo.split("-").map(Number) as [number, number]
  let y = fromYear
  let m = fromMonth
...
export class DreService {
  ...
  private async loadAccounts(): Promise<AccountLike[]> {
    if (this.useMock) {
  ...
  private async loadLines(): Promise<DbReportLine[]> {
    if (this.useMock) {
      return []
    }
  ...
  private async loadEntriesRange(
    periodFrom: string,
    periodTo: string,
  ...
  private async loadEntriesSinglePeriod(
    period: string,
    costCenters?: string[] | null,
    analysisType?: string | null
  ): Promise<EntryLike[]> {
  ...
  private async loadEntriesByAccount(code: string, period: string): Promise<EntryLike[]> {
    if (this.useMock) {
  ...
  async getCategories(): Promise<{ categories: DreCategory[] }> {
    const accounts = await this.loadAccounts()
    return { categories: buildCategoryTree(accounts) }
  }

  async getCostCentersData(): Promise<{ costCenters: DreCostCenter[] }> {
    if (this.useMock) {
      return { costCenters: [] }
    }
  ...
  async getReportLinesData(): Promise<{ lines: DreReportLine[] }> {
  ...
  async getDre(
    ...
    costCenterParam: string | undefined,
    ...
    const costCenters =
      costCenterParam && costCenterParam.trim().length > 0
    ...
    let comparativeValuesMap: Map<string, number> | null = null
    if (comparative) {
    ...
    let budgetMap: Map<string, number> | null = null
  ...
  async getDreMultiPeriod(
    ...
    const periodValuesMaps: Map<string, Map<string, number>> = new Map()
    ...
    for (const line of displayLines) {
      ...
      rows.push({
        ...
        nature: line.nature ?? "resultado",
        lineType: line.line_type as "account" | "computed" | "separator" | "header",
        values,
        avPercents,
      })
    }
  ...
  async getEvolution(months: number): Promise<{ points: DreEvolutionPoint[] }> {
    const now = new Date()
    const points: DreEvolutionPoint[] = []
    const accounts = await this.loadAccounts()
    const lines = await this.loadLines()
  ...
  async getDreDrill(code: string, period: string): Promise<DreDrillResponse> {
    const accounts = await this.loadAccounts()
    const account = accounts.find((a) => a.code === code)
  ...
  async exportCsv(
    ...
    const result = await this.getDre(
      period, periodFrom, periodTo, undefined, undefined, costCenterParam, analysisType, false
    )
  ...
  private computeLegacyValues(
    ...
    function computeValue(code: string): number {
      if (valuesMap.has(code)) return valuesMap.get(code)!
  ...
  private buildLegacyRows(
    ...
    const buildRow = (code: string): DreRow => {
      const account = accounts.find((a) => a.code === code)
      if (!account) {
        return {
          code,
      ...
      const avPercent = receitaLiquidaValue !== 0 ? (value / receitaLiquidaValue) * 100 : null
      const avPercentComparative =
        compValue !== undefined && compRL !== undefined && compRL !== 0
      ...
      return {
        ...
        nature: account.nature as "receita" | "deducao" | "custo" | "despesa" | "resultado",
        sign: account.sign,
        value,
        valueComparative: compValue,
        avPercent,
  ...
  private getLegacyLines(accounts: AccountLike[]): DbReportLine[] {
    const displayOrder = ["1", "RL", "2", "LB", "3", "EBITDA", "RAIRC", "RLIQ"]
